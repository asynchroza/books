...
def transform_pdf_to_epub(path_to_book):
    os.system(
        "ebook-convert {} {}".format(
            path_to_book.replace(" ", "\\ "),
            path_to_book.replace(".pdf", ".epub").replace(" ", "\\ "),
        )
    )
...
def send_book_to_mail(path_to_book):
    ...
    with open(path_to_book, "rb") as f:
        ...
        attach.add_header(
            "Content-Disposition",
            "attachment",
            filename=str(path_to_book[path_to_book.rfind("/") + 1 :]),
        )
        msg.attach(attach)
...
def write_in_readme():
    result = [
        y
        for x in os.walk(os.path.dirname(os.path.realpath(__file__)))
        for y in glob(os.path.join(x[0], "*.pdf"))
    ]

    with open("readme.md", "w") as readme:
        readme.write("<h3>PDF Books</h3>\n")
        for path in result:
            ...
            readme.write(
                "<li><a href='https://github.com/asynchroza/books/blob/main/{}/{}'>{}</a> - {}</li>\n".format(
                    base_dir, book_name_url, book_name, base_dir
                )
            )
